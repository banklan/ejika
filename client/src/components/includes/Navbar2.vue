<template>
  <nav>
    <v-app-bar app flat dark color="primary" height="68">
      <span class="hidden-md-and-up">
        <v-app-bar-nav-icon class="white--text hidden-md-and-up" @click="sidebar = true"></v-app-bar-nav-icon>
      </span>
      <v-toolbar-title class="white--text pl-5 ml-n3">
      ...
      <v-toolbar-items class="hidden-sm-and-down menu_items">
        ...
        <template v-if="authToken">
          <router-link
            v-for="item in authMenuItems" :key="item.title" :to="item.path">{{ item.title }}
          </router-link>
    ...
    <v-navigation-drawer app v-model="sidebar" color="primary" class="hidden-md-and-up" disable-resize-watcher>
